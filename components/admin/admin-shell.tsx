...
export function AdminShell({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-bone lg:flex-row">
      {/* Mobile bar */}
      <div className="flex items-center justify-between border-b border-border bg-bone-light px-4 py-3 lg:hidden">
        <Link href="/admin">
          <Logo className="h-8" />
        </Link>
        <button
      ...
      <aside
        id="admin-nav"
        className={cn(
          "shrink-0 border-border bg-bone-light lg:block lg:w-60 lg:border-r",
      ...
      >
                ...
                <li key={item.href}>
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-clay/15 text-graphite"
                        : "text-graphite/70 hover:bg-bone-light hover:text-graphite",
                    )}
                  >
        ...
        <div className="mt-6 border-t border-border px-5 py-4 lg:mt-auto">
          ...
          <button
            type="button"
            onClick={onSignOut}
            className="mt-3 inline-flex items-center gap-1.5 text-sm text-muted-foreground transition-colors hover:text-graphite"
          >
